...
const CinemaView = () => {
  ...
  return (
    <div className={`min-h-screen bg-black ${isChatBottom ? 'flex flex-col' : 'flex'}`}>
      ...
      <div className={`${isChatBottom ? 'h-3/5 min-h-[60vh]' : 'w-full'} flex items-center justify-center p-4`}>
      ...
      {/* Chat Integrado */}
      <div className={`${isChatBottom ? 'flex-shrink-0 h-1/2' : 'fixed right-0 top-0 h-full border-l border-gray-800 w-1/3 z-20'} bg-gray-900 transition-all duration-300`}>
        <EnhancedChat 
          creatorId={creatorId}
          messages={messages}
